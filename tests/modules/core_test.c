...
static char *test_core_conversions(void) {
  CoreTestCase tests[] = {
      {.name = "to-int from int is passthrough",
       .src = "(to-int 42)",
       .expected_str = "42",
       .expected_type = EXPECT_INT},
      {.name = "to-int truncates real toward zero",
       .src = "(to-int 3.9)",
       .expected_str = "3",
       .expected_type = EXPECT_INT},
      {.name = "to-int truncates negative real toward zero",
       .src = "(to-int -3.9)",
       .expected_str = "-3",
       .expected_type = EXPECT_INT},
      {.name = "to-int of ceil result",
       .src = "(import math [ceil]) (to-int (ceil 3.2))",
       .expected_str = "4",
       .expected_type = EXPECT_INT},
      {.name = "to-real from real is passthrough",
       .src = "(import math [ceil]) (to-real 1.5)",
       .expected_str = "1.5",
       .expected_type = EXPECT_REAL},
      {.name = "to-real promotes int",
       .src = "(to-real 7)",
       .expected_str = "7",
       .expected_type = EXPECT_REAL},
  };
