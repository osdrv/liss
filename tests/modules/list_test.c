...
static char *test_list_head_tail_last(void) {
    ListTestCase tests[] = {
        ...
        {.name = "tail of singleton is empty list",
         .src = "(import list [tail]) (is-empty? (tail [1]))",
         .expected_str = "true",
...
static char *test_list_append(void) {
    ListTestCase tests[] = {
        ...
        {.name = "append both empty",
         .src = "(import list [append]) (is-empty? (append [] []))",
         .expected_str = "true",
         .expected_type = EXPECT_BOOL},
    ...
    ListTestCase tests[] = {
        ...
        {.name = "recursive sum via head/tail",
         .src = "(import list [head tail])"
                "(fn sum [l] (cond (is-empty? l) 0 (+ (head l) (sum (tail l)))))"
                "(sum [1 2 3 4 5])",
         .expected_str = "15",
         .expected_type = EXPECT_INT},
...
static char *test_list_sort(void) {
    ListTestCase tests[] = {
        ...
        {.name = "sort empty list",
         .src = "(import list [sort]) (is-empty? (sort []))",
         .expected_str = "true",
         .expected_type = EXPECT_BOOL},
        ...
        {.name = "sort-by ascending",
         .src = "(import list [sort-by]) (sort-by [3 1 2] (fn [a b] (< a b)))",
         .expected_str = "[1 2 3]",
         .expected_type = EXPECT_LIST},
        {.name = "sort-by descending",
         .src = "(import list [sort-by]) (sort-by [1 3 2] (fn [a b] (> a b)))",
         .expected_str = "[3 2 1]",
         .expected_type = EXPECT_LIST},
        {.name = "sort-by on empty list",
         .src = "(import list [sort-by]) (is-empty? (sort-by [] (fn [a b] (< a b))))",
         .expected_str = "true",
         .expected_type = EXPECT_BOOL},
    };
    return run_list_tests(tests, sizeof(tests) / sizeof(tests[0]));
